...
// {
//  if (buffer == nullptr) {
//    std::cout << "ERROR : Buffer error. error code internalError 01"
//              << std::endl;
//    return;
//  }
//  if (!isalpha(symbol)) {
//    std::cout << "ERROR : Symbol error of volume, must A~Z. error code "
//                 "internalError 02"
//              << std::endl;
//    return;
//  }
//  if (bufferSize <= VOLUME_ROOT_SYMBOL_SIZE) {
//    std::cout << "ERROR : Buffer size error. error code internalError 01"
//              << std::endl;
//    return;
//  }
//  memcpy(buffer, VOLUME_ROOT_SYMBOL, VOLUME_ROOT_SYMBOL_SIZE * sizeof(WCHAR));
//  *(buffer + VOLUME_ROOT_SYMBOL_POS) = symbol;
//}

HANDLE CreateVolumeHandle(WCHAR volume[7]) {
  return CreateFile(volume, GENERIC_READ,
                    FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
                    NULL, OPEN_ALWAYS, FILE_FLAG_NO_BUFFERING, NULL);
}
...
HANDLE CreateVolumeHandle(WCHAR volumeWchar) {
  ...
  WCHAR volumeName[7] = {0};
  memcpy(volumeName, VOLUME_ROOT_SYMBOL,
         VOLUME_ROOT_SYMBOL_SIZE * sizeof(WCHAR));
  volumeName[VOLUME_ROOT_SYMBOL_POS] = volumeWchar;
  HANDLE handle =
      CreateFile(volumeName, GENERIC_READ,
                 FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                 OPEN_ALWAYS, FILE_FLAG_NO_BUFFERING, NULL);
  if (handle != INVALID_HANDLE_VALUE) {
    return handle;
  } else {
    std::cout << "ERROR : Create volume handle failed. error code"
                  "internalError 02"
              << std::endl;
    return handle;
  }
...
bool CreateUSNJouranl(HANDLE& volume) {
  ...
  bool retStatus =
      DeviceIoControl(volume, FSCTL_CREATE_USN_JOURNAL, &journalData,
                      sizeof(journalData), NULL, 0, 0, NULL);
  if (!retStatus) {
    std::cout << "ERROR : CreateUSNJouranl failed, error code : "
...
bool DeleteUSNJouranl(HANDLE& volume) {
  ...
  DWORD bytecount = 0;
  if (buffer == nullptr) {
    std::cout << "ERROR : Allocate virtual memory failed, error code : "
              << GetLastError() << std::endl;
    return false;
  }
  if (!QueryUSNJournal(volume, buffer, BUFFER_SIZE, bytecount)) {
    std::cout << "ERROR : QueryUSNJournal failed, error code : "
              << GetLastError() << std::endl;
  ...
  DELETE_USN_JOURNAL_DATA deleteJournalData{journal.UsnJournalID,
                                            USN_DELETE_FLAG_DELETE};
  // USN_DELETE_FLAG_DELETE or USN_DELETE_FLAG_NOTIFY
  bool retStatus =
      DeviceIoControl(volume, FSCTL_DELETE_USN_JOURNAL, &deleteJournalData,
                      sizeof(deleteJournalData), NULL, 0, 0, NULL);
  VirtualFree(buffer, 0, MEM_RELEASE);
  if (!retStatus) {
    std::cout << "ERROR : DeleteUSNJouranl failed, error code : "
              << GetLastError() << std::endl;
    return false;
  } else {
    return true;
...
bool GetNTFSVolumeInfo(HANDLE& volume, void*& buffer, DWORD bufferSize,
                       DWORD& retBytes) {
  bool retStatus = DeviceIoControl(volume, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0,
                                   buffer, bufferSize, &retBytes, NULL);
  return retStatus;
}
...
NTFS_EXTENDED_VOLUME_DATA GetNTFSVolumeInfo(HANDLE& volume) {
  ...
  DWORD retBytes = 0;
  bool retStatus = DeviceIoControl(volume, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0,
                                   buffer, BUFFER_SIZE, &retBytes, NULL);
  if (!retStatus) {
    std::cout << "ERROR : GetNTFSVolumeInfo failed, error code : "
              << GetLastError() << std::endl;
    return retData;
  }
...
bool QueryUSNJournal(HANDLE& volume, void*& buffer, DWORD bufferSize,
                     DWORD& bytecount) {
  bool retStatus = DeviceIoControl(volume, FSCTL_QUERY_USN_JOURNAL, NULL, 0,
...
bool QueryMFTEnumData(HANDLE& volume, MFT_ENUM_DATA_V0& mft, void*& buffer,
  ...
  bool retStatus =
      DeviceIoControl(volume, FSCTL_ENUM_USN_DATA, &mft, sizeof(mft), buffer,
...
//	DWORD bytecount = 1;
//
//	buffer = VirtualAlloc(NULL, BUFFER_SIZE, MEM_RESERVE | MEM_COMMIT,
// PAGE_READWRITE); 	if (buffer == NULL) { 		printf("VirtualAlloc:
// %u\n", GetLastError()); 		return;
//	}
//
...
//	if (parentRecord->FileReferenceNumber !=
// record->ParentFileReferenceNumber) { 		return;
//	}
